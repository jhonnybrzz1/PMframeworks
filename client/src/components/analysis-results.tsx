...
Gerado por Frameworks - Análise Crítica para PMs
  ...
  const handleExportPDF = async () => {
    ...
    try {
      // Criar payload para envio à API
      const pdfData = {
        analysis,
        inputText,
        timestamp: Date.now()
      ...
      // Chamar a API para gerar o PDF no servidor com Playwright
      ...
      // Fallback para a implementação anterior com jsPDF
      console.warn('Falha ao gerar PDF via API. Usando método alternativo com jsPDF...');
      ...
      const addSection = (title: string, content: string | string[], color: [number, number, number]) => {
        ...
        if (Array.isArray(content)) {
          content.forEach((item, index) => {
            ...
            yPosition = addWrappedText(`• ${cleanItem}`, 9);
            yPosition += 3; // Space between items
          });
        } else {
          yPosition = addWrappedText(content, 9);
        }
      ...
      pdf.setFontSize(18);
      pdf.setFont('helvetica', 'bold');
      pdf.text('Análise Crítica - Frameworks PM', margin, 18);

      pdf.setFontSize(10);
      pdf.setFont('helvetica', 'normal');
      pdf.text(`Framework: ${analysis.framework}`, margin, 25);

      // Add document name if available
      const docName = inputText?.substring(0, 60).replace(/[^\w\s]/g, '').trim() || 'Documento';
      pdf.text(`Documento: ${docName}...`, margin, 30);

      // Date
      const now = new Date();
      pdf.text(`Data: ${now.toLocaleDateString('pt-BR')}`, pageWidth - margin - 45, 25);
      ...
      toast({
        title: "PDF Exportado (Fallback)",
        description: "Análise exportada como PDF com método alternativo.",
      });
    }
  };
